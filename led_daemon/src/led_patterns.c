	...
	struct led_state_st * const state, struct blob_attr const * const attr)
{
	...
	struct blobmsg_policy const led_policy[__LED_MAX] = {
		[LED_NAME] =
	...
	struct pattern_step_st * const new_step, struct blob_attr const * const attr)
{
	...
	struct blobmsg_policy const step_policy[__STEP_MAX] = {
		[STEP_TIME_MS] =
		{.name = "time_ms", .type = BLOBMSG_TYPE_INT32 },
		[STEP_LEDS] =
		{.name = "leds", .type = BLOBMSG_TYPE_ARRAY }
	};
	struct blob_attr *fields[__STEP_MAX];
...
parse_led_pattern(struct blob_attr const * const attr)
{
	...
	struct blobmsg_policy const pattern_policy[__PATTERN_MAX] = {
		[PATTERN_NAME] =
	...
	void * const user_ctx)
{
	...
	struct blobmsg_policy const patterns_policy[__PATTERNS_MAX] = {
		[PATTERNS] =
			{ .name = "patterns", .type = BLOBMSG_TYPE_ARRAY }
	};
	struct blob_attr *fields[__PATTERNS_MAX];
