    ...
    struct blob_buf * const response)
{
    ...
    struct blob_attr * fields[GET_MAX__];
    ...
    {
        if (!process_get_state_attr(led_ops, led_ops_handle, cur, response))
        {
            result = UBUS_STATUS_INVALID_ARGUMENT;
            goto done;
        }
    ...
    struct blob_buf * const response)
{
    ...
    if (set_state_req.led_name == NULL || set_state_req.state == LED_STATE_UNKNOWN)
    {
        success = false;
        goto done;
    }
    ...
    struct blob_buf * const response)
{
    struct led_ops_st const * const led_ops = ubus_context->led_ops;
    int result;
    struct blob_attr * fields[SET_MAX__];
    struct led_ops_handle_st * const led_ops_handle =
        led_ops->open(ubus_context->led_ops_context);
...
{
    int result;
    struct blob_attr * fields[ACTIVATE_MAX__];
    struct led_ops_st const * const led_ops = ubus_context->led_ops;
    struct led_ops_handle_st * const led_ops_handle =
        led_ops->open(ubus_context->led_ops_context);
    ...
    struct blob_buf * const response)
{
    ...
    struct blob_attr * fields[LOCK_MAX__];
...
{
    int result;
    struct blob_attr * fields[DEACTIVATE_MAX__];
    struct led_ops_st const * const led_ops = ubus_context->led_ops;
    struct led_ops_handle_st * const led_ops_handle =
        led_ops->open(ubus_context->led_ops_context);
    ...
    struct blob_buf * const response)
{
    ...
    for (enum led_state_t state = LED_STATE_FIRST; state < LED_STATE_MAX; state++)
    {
...
process_pattern_play_msg(
    ...
    struct blob_buf * const response)
{
    struct blob_attr * fields[PATTERN_PLAY_MAX__];
...
process_pattern_stop_msg(
    ...
    struct blob_buf * const response)
{
    struct blob_attr * fields[PATTERN_STOP_MAX__];

    blobmsg_parse(pattern_stop_policy, ARRAY_SIZE(pattern_stop_policy), fields,
                  blobmsg_data(msg), blobmsg_len(msg));

    char const * const pattern_name = blobmsg_get_string(fields[PATTERN_STOP_NAME]);
    struct led_ops_st const * const led_ops = ubus_context->led_ops;
