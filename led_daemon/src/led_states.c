...
{
    return state >= LED_STATE_FIRST && state < ARRAY_SIZE(led_states);
}
...
led_state_entry_lookup(char const * const name)
{
    ...
    for (enum led_state_t i = LED_STATE_FIRST; i < ARRAY_SIZE(led_states); i++)
    {
...
{
    struct led_state_st const * const state_entry = led_state_entry_lookup(name);
