	...
	struct led_pattern_st const * const led_pattern)
{
	...
	TAILQ_INSERT_TAIL(
		&patterns_context->playing_patterns, pattern_context, entry);
}
...
led_pattern_start(struct led_pattern_context_st * const pattern_context)
{
	...
	bool const start_step_completed =
		!start_step_required
		|| led_pattern_play_start_step(pattern_context);
	...
	struct led_pattern_st const * const pattern)
{
	...
	if (led_used_by_step(
			patterns_context, led_state, &pattern->end_step)) {
		used_by_pattern = true;
		goto done;
	}
	...
	for (size_t i = 0; i < pattern->num_steps; i++) {
		if (led_used_by_step(
				patterns_context, led_state, &pattern->steps[i])) {
			used_by_pattern = true;
			goto done;
		}
	}
	...
	struct led_pattern_st const * const pattern)
{
	...
	for (size_t i = 0; i < step->num_leds; i++) {
		if (led_used_by_pattern(
				patterns_context, &step->leds[i], pattern)) {
			shares_led = true;
			goto done;
		}
	}
