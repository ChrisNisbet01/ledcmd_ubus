...
struct ledcmd_ctx_st {
	...
	bool supported_states[__LED_STATE_MAX];
	...
	enum led_priority_t const priority)
{
	enum led_priority_t const new_highest_priority =
		led_priority_priority_deactivate(
			led_ctx->priority_context, priority);
...
update_flashing(struct flash_context_st * const flash_ctx)
{
	...
	if (!flash_ctx->flash_forever && flash_ctx->remaining_time_ms == 0) {
		...
		flash_ctx->current_time =
			(next_state == LED_ON)
			? flash_ctx->times->on_time_ms
			: flash_ctx->times->off_time_ms;
	}
	...
	struct set_state_req_st const * const request)
{
	...
	if (request->flash_type == LED_FLASH_TYPE_ONE_SHOT) {
		...
		flash_ctx->current_time =
			(request->flash_time_ms > 0)
			? request->flash_time_ms
			: flash_ctx->times->on_time_ms;
	...
	void * result_context)
{
	...
	if (led_alias != NULL) {
		...
		led_alias_iterate(
			led_alias, led_alias_set_state_cb, (void *)&set_state_alias);
	}
	...
	void * result_context)
{
	...
	if (led_alias != NULL) {
		...
		led_alias_iterate(
			led_alias, led_alias_get_state_cb, (void *)&get_state_alias);
	}
...
led_alias_deactivate_cb(char const * const led_name, void * user_ctx)
{
	...
	result_cb(
		led_name, unlocked, led_ctx->lock_id, error_msg, result_context);
	...
	void * const result_context)
{
	...
	if (led_alias != NULL) {
		...
		led_alias_iterate(
			led_alias, led_alias_deactivate_cb, (void *)&activate_alias);
	}
	...
	void * const result_context)
{
	...
	if (led_alias != NULL) {
		...
		led_alias_iterate(
			led_alias, led_alias_activate_cb, (void *)&activate_alias);
	}
	...
	void * const result_context)
{
	...
	led_aliases_iterate(
		context->led_aliases, list_aliases_cb, (void *)&aliased_leds);
	...
	char const * const led_name)
{
	...
	led_alias_iterate(
		led_alias, compare_aliased_leds_cb, &compare_aliased_leds);
	...
	char const * const led_b_name)
{
	bool found_match;
	led_alias_st const * const led_alias =
		led_alias_lookup(context->led_aliases, led_a_name);
	...
	led_alias_iterate(
		led_alias, compare_led_to_aliased_leds_cb, &led_to_aliased_leds);
	...
	char const * const led_b_priority)
{
	...
	bool const got_priority_a =
		led_priority_by_name(led_a_priority, &priority_a);
	enum led_priority_t priority_b = 0;
	bool const got_priority_b =
		led_priority_by_name(led_b_priority, &priority_b);
...
{
	struct led_ops_handle_st *led_ops_handle =
		calloc(1, sizeof *led_ops_handle);
	...
	void * result_context)
{
	...
	if (set_all) {
		...
		bool const found_aliased_leds =
			set_aliased_led_states(
				context, led_handle, request, result_cb, result_context);
		struct led_ctx_st * const led_ctx =
			avl_find_element(
				&context->all_leds, request->led_name, led_ctx, node);
		char const * error_msg = NULL;
	...
	void * result_context)
{
	...
	if (get_all_leds) {
		...
		bool const found_aliased_leds =
			get_aliased_led_states(
				context, led_handle, led_name, result_cb, result_context);
		struct led_ctx_st * const led_ctx =
			avl_find_element(&context->all_leds, led_name, led_ctx, node);
	...
	if (do_all) {
		...
		bool const deactivated_aliased_leds =
			deactivate_aliased_leds(
				...
				result_context);
		struct led_ctx_st * const led_ctx =
			avl_find_element(&context->all_leds,
							 led_name,
							 led_ctx,
							 node);
	...
	void * const result_context)
{
	...
	if (do_all) {
		 ...
		 */
		if (priority == LED_PRIORITY_LOCKED
				&& ledcmd_ctx_any_led_locked(context)) {
...
led_ops_list_patterns(
	void * const led_ops_context,
	list_patterns_cb const result_cb,
	void * const user_ctx)
{
	struct ledcmd_ctx_st * const context = led_ops_context;
	...
	void * const user_ctx)
{
	...
	bool const success =
		led_pattern_stop_pattern(
			context->patterns_context, pattern_name, &error_msg);
...
leds_init(led_st * const led, void * const user_ctx)
{
	...
	led_ctx->priority_context =
		led_priority_allocate(LED_PRIORITY_COUNT);
	...
	char const * const backend_directory)
{
	...
	context->patterns_context =
		led_patterns_init(patterns_directory, &ops, context);
